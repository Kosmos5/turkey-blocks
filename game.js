...
(function () {
  ...
  var DEFAULT_CFG = {
    ...
    rampEvery: 30,        // 每涨 N 行难度 +1 档（0 = 关闭）
    minRows: 2,           // 场上「有格子的行数」少于这个数时，自动从底部补行
    emptyTilt: 0.35,      // 难度对「空格数」的倾斜：每档让新行更偏向 emptyMax 那一端
    advTighten: 2,        // 每升 1 档，困难行间隔缩短几行（有下限 4）
    levelWeights: null,   // 每个档位的 [1格,2格,3格,4格] 权重表；null = 用默认公式生成
    ...
    combo4: 1.6,
    combo5: 2.0,
    baseWeights: { 1: 34, 2: 30, 3: 22, 4: 14 },   // 新行里各方块长度的基础权重
  ...
  function loadCfg() {
    ...
    if (saved && typeof saved === 'object') {
      for (var k in c) {
        if (k === 'baseWeights') {
          if (saved.baseWeights) for (var w in c.baseWeights) if (typeof saved.baseWeights[w] === 'number') c.baseWeights[w] = saved.baseWeights[w];
  ...
  // 后段难度：长条权重上升、困难行更频繁
  // 每档用哪套权重由 CFG.levelWeights 决定（设置面板里可逐档编辑）
  var DEFAULT_LEVELS = 8;
  function defaultLevelWeights() {
    var out = [], L, b = CFG.baseWeights;
    for (L = 0; L < DEFAULT_LEVELS; L++) {
      out.push([
        Math.max(4, (b[1] || 34) - 5 * L),
        Math.max(6, (b[2] || 30) - 1 * L),
        (b[3] || 22) + 3 * L,
        (b[4] || 14) + 5 * L
      ]);
    }
  ...
  // 新行的空格数：范围固定在 emptyMin~emptyMax，但难度越高越偏向「多空格」那一端
  function pickEmpty() {
    var lo = clamp(CFG.emptyMin | 0, 1, 8);
    var hi = clamp(CFG.emptyMax | 0, lo, 8);
    if (hi === lo) return lo;
    var lv = tideLevel(), w = {}, k;
    for (k = lo; k <= hi; k++) w[k] = 1 + lv * (k - lo) * CFG.emptyTilt;
    return weightedPick(w);
  }

  // 一把长度总和正好等于 fill 的方块（长度 1~4，相邻尽量不同色）
  function makeLens(fill, w, forceFirst) {
    var lens = [], sum = 0, k;
    if (forceFirst) { lens.push(forceFirst); sum = forceFirst; }
    var guard = 0;
    while (sum < fill && guard++ < 60) {
      var room = fill - sum, last = lens[lens.length - 1];
      var ww = {};
      for (k in w) if (Number(k) <= room && Number(k) !== last) ww[k] = w[k];
      if (!Object.keys(ww).length) for (k in w) if (Number(k) <= room) ww[k] = w[k];
  ...
  function settingsHtml() {
    ...
    return '<h2>数值设置</h2><p class="muted">点击「保存并重开」后立即生效。所有数值都存在浏览器本地。</p>' +
      ...
      num('cfgEmptyTilt', '难度对空格的倾斜（越大后期空洞越多）', CFG.emptyTilt, 0.05, 0) +
